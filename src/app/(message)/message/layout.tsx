...
export default function MessageLayoutWrapper({ children }: { children: React.ReactNode }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (profile?._id) {
          ...
          const rooms = await myFetch("/chat-room", { method: "GET", tags: ["chat"] });
  ...
  return (
    <div className="flex h-full overflow-hidden bg-[#0a0a10]">
      {/* Sidebar - Persistent */}
      <div className="w-full lg:w-1/3 xl:w-1/4 shrink-0 h-full">
        <ChatSidebar chatRooms={chatRooms} currentUserId={currentUserId} />
      </div>
